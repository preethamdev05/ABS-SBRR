...
class BellController:
    ...
    def ring(self, pattern: str = 'single_ring', duration_seconds: int = 3):
        ...
        Duration is capped at max_bell_duration from config."""
        if self._state != _IDLE:
            return
        max_dur = self._cfg.get('max_bell_duration', _MAX_DUR_S)
        duration_seconds = max(1, min(int(duration_seconds), max_dur))
